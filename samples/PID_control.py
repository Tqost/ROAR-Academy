    ...
    def compute_error(self, track):
        ...
        for i in range(len(track)):
            ...
            if distance < min_distance:
                ...
                error = (
                    self.y - track[i][1]
                )  # This error only assumes cte based on y-axis. Further extension is possible
...
vehicle = Vehicle2D()
vehicle.set(0, 1, 0)
vehicle.set_steering_drift(10 / 180.0 * np.pi)
...
controller_legend = (
    "PID controller"
    if K_p != 0 and K_d != 0 and K_i != 0
    else (
        "PD controller"
        if K_p != 0 and K_d != 0
        else "P controller" if K_p != 0 else "unknown controller"
    )
)
